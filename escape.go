...
func (b *Buffer) appendQuotedString(s string) {
	...
	// Alias the string's bytes for indexing and bulk copy without conversion.
	...
	for i < n {
		...
		if c < 0x20 || c == '"' || c == '\\' {
			if i > start {
				b.buf = append(b.buf, data[start:i]...)
			}
			appendASCIIEscape(b, c)
			i++
			start = i
			continue
		}
	...
	if start < n {
		b.buf = append(b.buf, data[start:]...)
	}
	b.buf = append(b.buf, '"')
}

func appendASCIIEscape(b *Buffer, c byte) {
	switch c {
	case '"':
		b.buf = append(b.buf, '\\', '"')
	case '\\':
